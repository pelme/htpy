...
def test_iter_chunks_coroutine() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=(
            r"<coroutine object .+> can not be used in sync context\. "
            r"Use \.aiter_chunks\(\) to retrieve the content: .+"
        ),
    ):
        list(ul[coroutine()].iter_chunks())


def test_iter_chunks_async_generator() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=(
            r"<async_generator object .+> can not be used in sync context\. "
            r"Use \.aiter_chunks\(\) to retrieve the content: .+"
        ),
    ):
        list(ul[generator()].iter_chunks())
